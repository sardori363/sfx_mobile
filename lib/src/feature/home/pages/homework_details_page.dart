...
class _HomeWorkDetailsPageState extends State<HomeWorkDetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BlocListener<HWDetailsBloc, HWDetailsState>(
        listener: (context, state) {
          if (state.pageState == HWDetailsPageState.goToSuccess) {
            context.go(AppRouteName.successPage);
          } else if (state.pageState == HWDetailsPageState.error) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Failed to submit images')),
            );
          }
        },
        child: BlocBuilder<HWDetailsBloc, HWDetailsState>(
              ...
              return Row(
                children: [
                  ...
                  Expanded(
                    child: MaterialButton(
                      onPressed: () async {
                        BlocProvider.of<HWDetailsBloc>(context).add(SubmitImagesEvent(widget.currentTask.id));
                      },
                      height: 40.h,
                      padding: const EdgeInsets.all(8),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)),
      ...
      body: BlocBuilder<HWDetailsBloc, HWDetailsState>(
        builder: (context, state) {
          ...
          if(state.pageState == HWDetailsPageState.loading){
